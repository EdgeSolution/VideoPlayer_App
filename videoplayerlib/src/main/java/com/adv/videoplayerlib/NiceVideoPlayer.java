...
import static com.adv.videoplayerlib.NiceUtil.test;
...
public class NiceVideoPlayer extends FrameLayout
    ...
    public final String TAG = "NiceVideoPlayer";
    ...
    public static List<String> mPlayVideoList;
    public static List<String> mVideoList;
    public static ReadWriteLock numberLock;
    private ReadWriteLock loopLock;
    ...
    private void init() {
        numberLock = new ReentrantReadWriteLock();
        loopLock = new ReentrantReadWriteLock();
    ...
    public static void setVideoNumber(int tmp) {
    ...
    public void setLoopState(boolean value) {
        ...
        try {
            ...
            loopLock.writeLock().unlock(); //释放读锁
        }
        ...
        if (mMediaPlayer == null) {
            switch (mPlayerType) {
                ...
                default:
                    ...
                    ((IjkMediaPlayer) mMediaPlayer).setOption(IjkMediaPlayer.OPT_CATEGORY_FORMAT, "reconnect", 1);
                    ((IjkMediaPlayer) mMediaPlayer).setOption(IjkMediaPlayer.OPT_CATEGORY_PLAYER, "min-frames", 100);
                    ((IjkMediaPlayer) mMediaPlayer).setOption(IjkMediaPlayer.OPT_CATEGORY_PLAYER, "max-buffer-size", 100 * 1024);
...
//                    ((IjkMediaPlayer)mMediaPlayer).setOption(4, "framedrop", 1L);
    ...
    public void onSurfaceTextureAvailable(SurfaceTexture surfaceTexture, int width, int height) {
        ...
        if (mSurfaceTexture == null) {
            if (mPlayVideoList != null && mPlayVideoList.size() != 0) {
    ...
    private IMediaPlayer.OnCompletionListener mOnCompletionListener
        ...
        public void onCompletion(IMediaPlayer mp) {
            ...
            Log.e(TAG, "global_number is call.");
            loopLock.readLock().lock();
            try {
                if (isLoop) {
                    numberLock.writeLock().lock();
                    try {
                        global_number++;
                        if (global_number == mPlayVideoList.size()) {
                            global_number = 0;
                        }
                    } finally {
                        numberLock.writeLock().unlock(); //释放读锁
                    }
    ...
    public void enterTinyWindow() {
        ...
        params.topMargin = NiceUtil.dp2px(mContext, 8f);
        params.leftMargin = NiceUtil.dp2px(mContext, 8f);
